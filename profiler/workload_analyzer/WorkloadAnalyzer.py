...
"""
Get the test start time from the output log of SWI
"""
def GetTestMetadata(test_metadata_file=FAAS_ROOT+"/synthetic_workload_invoker/test_metadata.out"):
    ...
    with open(test_metadata_file) as f:
        ...
        print('Total # of invocations by Workload Invoker:', invoked_actions)
    try:
...
"""
Extracts details from activation json records
"""
...
"""
Generate a dataframe which describes the test in a standard format
"""
...
"""
Construct a dataframe for the performance information of all invocations
"""
def ConstructTestDataframe(since, limit=1000, read_results=False):
    from ContactDB import GetActivationRecordsSince
    perf_data = {
        'func_name': [], 'activationId': [], 'start': [], 'end': [],
        'duration': [], 'waitTime': [], 'initTime': [], 'latency': [],
        'lang': []
    }
    if read_results:
        perf_data['results'] = []
    ...
    for activation in activations:
        ...
        perf_data['latency'].append(perf_data['duration'][-1]+perf_data['waitTime'][-1])
        if read_results:
...
"""
Create a dataframe including the statistical summary of function invocations in the test
"""
def CreateStatisticalSummary(test_df, config_df, test_start_time):
    ...
    for function in functions:
        ...
        try:
            ...
            start = config_df[config_df['application'] == function]['activity_window'].tolist()
        ...
        try:
            throughput = 1000.0 * len(partial_df['start'])/(partial_df['end'].max() - start_time)
        except:
            throughput = 0
...
def GetSystemdCgtopDetails(file_path):
    ...
    return {'container_count': container_count, 'docker_tasks': docker_tasks, 'memory': memory}

"""
Reads the systemd-cgtop records
"""
def GetControlGroupsRecords(since=None):
    cgroups_rec = {'timestamp': [], 'container_count': [], 'docker_tasks': [], 'memory': []}
    if since == None:
        logger.error("No since parameter entered!")
        return None
    log_path = FAAS_ROOT+'/logs'
    scg_files = [f for f in os.listdir(log_path) if (isfile(join(log_path, f))) and ('systemd-cgtop' in f)]

    # only keep the recent records
    scg_files = [f for f in scg_files if int(f[f.index('_')+1:f.index('.')]) >= since]

    if len(scg_files) == 0:
        logger.info("There are no applicable systemd-cgtop records in the logs directory.")
        return None
...
"""
Returns the capacity factor for a workload, meaning how much it is stressing the system based on invocation latencies
"""
def CapacityFactor(test_df):
    ...
    for function in functions:
        ...
        capacity_factors[function] = temp_data['latency'].quantile(0.9) - temp_data['latency'].quantile(0.1)

    print('CapacityFactor (90th-percentile - 10th-percentile latency): ' + str(capacity_factors))
...
def main(argv):
    ...
    parser.add_option("-r", "--read_results", dest="read_results",
                      help="gather the results of function invocations along with the activation details", action='store_true')
    ...
    print('[DEBUG] test_start_time:', test_start_time, 'config_file:', config_file)
    ...
    test_df = ConstructTestDataframe(since=test_start_time, limit=100000, read_results=read_results)
    if (test_df is None):
        logger.error('Test result dataframe could not be constructed!')
        return False
    print('Total # of records read from CouchDB: ' + str(len(test_df['start'])))
    ...
    try:
        ...
        print('Avg invocation period: ' + str(mean_invocation_period), 'ms')
    ...
    print('Statistics Summary Dataframe:\n', stat_df, '\nrel_stress = rate/throughput\n')
    ...
    if options.plot:
        ...
        TestDataframePlotter(save_plot, test_df, test_name, cgroups_df)
        # PerfMonPlotter(perf_mon_records, time_window=[5, 10])
    if options.archive:
        now = datetime.now()
        file_name = FAAS_ROOT + '/data_archive/' + now.strftime("%Y_%m_%d_%H_%M") + '_' + test_name + '.pkl'
        pickle.dump([test_name, config_df, stat_df, test_df, perf_mon_records], open(file_name, "wb"))
    ...
    print('\n==== Performance Summary ====')
    for dim in ['waitTime', 'initTime', 'execution', 'latency']:
        print('Mean ' + dim + ' (ms):', test_df[dim].mean())
        print('Std ' + dim + ' (ms):', test_df[dim].std())
        print('95% ' + dim + ' (ms):', test_df[dim].quantile(.95))
        print('99% ' + dim + ' (ms):', test_df[dim].quantile(.99))
        print('99.9% ' + dim + ' (ms):', test_df[dim].quantile(.999))
        print('***********')
    print()
    warm_starts_test_df = test_df[test_df['initTime'] == 0]
    print('Warm Start Performance Summary (count: ' + str(len(warm_starts_test_df[dim])) + ')')
    for dim in ['waitTime', 'execution', 'latency']:
        print('Mean ' + dim + ' (ms):', warm_starts_test_df[dim].mean())
        print('Std ' + dim + ' (ms):', warm_starts_test_df[dim].std())
        print('95% ' + dim + ' (ms):', warm_starts_test_df[dim].quantile(.95))
        print('99% ' + dim + ' (ms):', warm_starts_test_df[dim].quantile(.99))
        print('99.9% ' + dim + ' (ms):', warm_starts_test_df[dim].quantile(.999))
        print('***********')
    print()
    cold_starts_test_df = test_df[test_df['initTime'] != 0]
    print('Cold Start Performance Summary (count: ' + str(len(cold_starts_test_df[dim])) + ')')
    if len(cold_starts_test_df) != 0:
        for dim in ['waitTime', 'initTime', 'execution', 'latency']:
            print('Mean ' + dim + ' (ms):', cold_starts_test_df[dim].mean())
            print('Std ' + dim + ' (ms):', cold_starts_test_df[dim].std())
            print('95% ' + dim + ' (ms):', cold_starts_test_df[dim].quantile(.95))
            print('99% ' + dim + ' (ms):', cold_starts_test_df[dim].quantile(.99))
            print('99.9% ' + dim + ' (ms):', cold_starts_test_df[dim].quantile(.999))
            print('***********')
        print()

    print('\nFor evaluation purpose:')
    print(str(round(warm_starts_test_df['latency'].mean(), 1)) + ',' + str(round(warm_starts_test_df['latency'].std(), 1)) + ',' + str(round(warm_starts_test_df['latency'].quantile(.95), 1)) + ',' + str(round(warm_starts_test_df['latency'].quantile(.99), 1)) + ',' +\
          str(round(warm_starts_test_df['waitTime'].mean(), 1)) + ',' + str(round(warm_starts_test_df['waitTime'].quantile(.99), 1)) + ',' +\
          str(round(warm_starts_test_df['execution'].mean(), 1)) + ',' + str(round(warm_starts_test_df['execution'].quantile(.99), 1)) + ',' +\
          str(round(stat_df['throughput'][0], 2)))
    with open('results.csv', 'a') as f:
        f.write(str(round(warm_starts_test_df['latency'].mean(), 1)) + ',' + str(round(warm_starts_test_df['latency'].std(), 1)) + ',' + str(round(warm_starts_test_df['latency'].quantile(.95), 1)) + ',' + str(round(warm_starts_test_df['latency'].quantile(.99), 1)) + ',' +\
          str(round(warm_starts_test_df['waitTime'].mean(), 1)) + ',' + str(round(warm_starts_test_df['waitTime'].quantile(.99), 1)) + ',' +\
          str(round(warm_starts_test_df['execution'].mean(), 1)) + ',' + str(round(warm_starts_test_df['execution'].quantile(.99), 1)) + ',' +\
          str(round(stat_df['throughput'][0], 2)) + '\n')
